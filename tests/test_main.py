...
class TestGetMove(unittest.TestCase):
    ...
    def test_get_move_non_numeric(self, mock_input):
        """Tests that the function handles non-numeric input ('hello') by looping once and then accepting the valid input '5'."""
        mock_input.side_effect = ["hello", "5"]
        result = main.get_move()
        self.assertEqual(result, (1,1))
    
    def test_get_move_valid_input(self, mock_input):
        """Tests the base case of receiving a single, valid numeric input ('8') and verifying the correct coordinates (2, 1)."""
...
class TestGetWinner(unittest.TestCase):
    ...
    def test_get_winner_empty_board(self):
        """Tests that a brand new, empty board (all None values) is correctly identified as having no winner (returns None)."""
        board = main.new_board()
        result = main.get_winner(board)
        self.assertIsNone(result)

    def test_get_winner_almost_win(self):
        """Tests a partial board setup containing multiple two-in-a-row patterns to ensure no premature winner is declared (returns None)."""
        board = [["X", "X", None], 
                 ["O", "O", None], 
                 [None, "X", "O"]]
        result = main.get_winner(board)
