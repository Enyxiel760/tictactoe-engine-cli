"""Unit tests for the RandomAIPlayer.

This module verifies the behavior of the RandomAIPlayer, specifically ensuring that it correctly
identifies available moves and delegates selection to the random number generator.
"""

import unittest
from unittest.mock import patch

from src.players import RandomAIPlayer
...
class TestRandomAIPlayer(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        This also implicitly validates the abstract base class contract: if RandomAIPlayer fails
        to implement any required abstract methods, instantiation will raise a TypeError here.
        """
        cls.player = RandomAIPlayer(name="Test", marker="X")
        cls.engine = MockEngine()

    def test__calculate_move(self, mock_choice):
        """Verify correct move selection logic.
